...
def main():
    ...
    if choice == menu[0]:
        ...
        file = st.file_uploader('이미지 파일을 선택하세요.', type=['jpg', 'png', 'jpeg', 'WebP'])

        # 유저가 올린 파일이 있을 때만 서버에 저장한다.
        if file is not None:
            print(file.name)
            print(file.size)
            print(file.type)
            ...
            # 현재 시간과 유저의 아이디를 조합해서 만드는게 실무에서 가장 많이 사용
